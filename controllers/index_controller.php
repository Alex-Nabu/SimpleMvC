...
public function __construct(object_factory_inc $factory,array $parameters )
 {
  ...
  {
	  if(isset($_SESSION['loggedin']))//if logged in session is set;
	  {
		  if(empty($this->parameters))
		  {
			  $this->parameters['loggedin']='loggedin';
		  }
		  
		  else  //if the parameters arent empty, preserve the other vlaues and add the loggedin argument to the top to be recognized in the index controller builder
		  {
			  array_unshift($this->parameters,"loggedin");
			  
		  }
		  
		  throw new Exception("loggedin");
	  }
